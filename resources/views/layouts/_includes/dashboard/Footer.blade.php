<footer class="py-1 mt-4 bg-primary">
    ...
    <p class="text-center mt-3 text-white">
        <span>&copy; {{ date('Y') }}, portal da Associação Nacional dos Profissionais do Espectro Rádio Electrónico
        </span> by <a href="https://infosi.gov.ao/" target="_blank" class="text-white">INFOSI</a>
    </p>
</footer>
@if (session('create'))
    <script>
...
<script>
    ...
    CKEDITOR.replace('negotiatedPrices');
    CKEDITOR.replace('negotiatedPrices');
    CKEDITOR.replace('contacts');
    CKEDITOR.replace('beneficiaries');
    CKEDITOR.replace('location');
    CKEDITOR.replace('designationProject');
</script>
